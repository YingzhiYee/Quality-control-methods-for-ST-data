...
class FaceUtil:
    ...
    def load_embeddings(self, encoding_file_path):
        ...
        print("[INFO] loading featureInfo encodings...")
        self.data = pickle.loads(open(encoding_file_path, "rb").read())

    # featureInfo detection
    ...
    # featureInfo recognition
    def get_face_location_and_name(self, image):
        # convert the input frame from BGR to RGB
        rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        # detect the (x, y)-coordinates of the bounding boxes
        # corresponding to each featureInfo in the input frame, then
        # compute the facial embeddings for each featureInfo
        boxes = face_recognition.face_locations(
            rgb, model=self.detection_method)
        encodings = face_recognition.face_encodings(rgb, boxes)

        # initialize the list of names for each featureInfo detected
        names = []

        # loop over the facial embeddings
        for encoding in encodings:
            # attempt to match each featureInfo in the input image to
            # our known encodings
            ...
            if True in matches:
                ...
                #  of times each featureInfo was matched
                matched_idxs = [i for (i, b) in enumerate(matches) if b]
                counts = {}

                # loop over the matched indexes and maintain a count
                # for each recognized featureInfo
                for i in matched_idxs:
                    name = self.data["names"][i]
                    counts[name] = counts.get(name, 0) + 1

                # determine the recognized featureInfo with the largest
    ...
    def save_embeddings(self, image_paths, output_encoding_file_path):
        ...
        for (i, image_path) in enumerate(image_paths):
            ...
            # detect the (x, y)-coordinates of the bounding boxes
            # corresponding to each featureInfo in the input image
            boxes = face_recognition.face_locations(rgb, model=self.detection_method)

            # compute the facial embedding for the featureInfo
            encodings = face_recognition.face_encodings(rgb, boxes)
